"""Smoke test: daily_digest Kanban handler wraps daily_digest_job correctly."""
import sys
import types
import unittest
...
import tools.digest.agent  # noqa: E402


class TestKanbanDailyDigestHandler(unittest.IsolatedAsyncioTestCase):

    async def test_handle_passes_payload_to_job(self):
        from workers.task_handlers.daily_digest import handle

        task = {"Id": 1, "task_type": "daily_digest", "input_payload": {"org_id": 1}}
        expected = {"status": "ok", "digest_id": 10}

        with patch("tools.digest.agent.daily_digest_job", return_value=expected) as mock_job:
    ...
    async def test_handle_empty_payload(self):
        ...
        task = {"Id": 2, "task_type": "daily_digest"}
    ...
    async def test_handle_propagates_exception(self):
        ...
        task = {"Id": 3, "task_type": "daily_digest", "input_payload": {"org_id": 1}}

        with patch("tools.digest.agent.daily_digest_job", side_effect=RuntimeError("llm error")):
